...
cocoaMultiVis <- function(rsEnSortedInd, GRList, coordinateDT, loadingMat, rsEnrichment,
                          ...
                          makeRQBPC=TRUE, 
                          topRSInd_rQBPC = unique(unlist(rsEnSortedInd[1:10, ])),
                          PCsToAnnotate_rQBPC = c(PCSTOANNOTATE),
                          makePCFSCH=TRUE, 
                          topRSInd_pcFSCH = unique(unlist(rsEnSortedInd[1:10, ])), 
    ...
    if (!is.null(pMeta) && !is.null(colorByCols)) {
        ...
        pcScores = mPCA$x
        colorTSNE = colorClusterPlots(clusteredDF = cbind(pcScores, pMeta), 
                                      plotCols = c("PC1", "PC2"), colorByCols = colorByCols)
        ggplot2::ggsave(filename=paste0(getPlotSubdir(paste0(regionSetID[i], "_", inputID)), ".pdf"), 
                        plot = colorTSNE, device = "pdf",
                        limitsize=FALSE)
    }
