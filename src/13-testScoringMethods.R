...
#########################################################################
# visualize loading distributions

grDevices::pdf(file=ffPlot(paste0(plotSubDir, "loadingDist.pdf")))
for (i in 1:15) {
    hist(mPCA$rotation[, i], main = paste0("PC", i))
    hist(abs(mPCA$rotation[, i]), main = paste0("Abs() PC", i))
}
